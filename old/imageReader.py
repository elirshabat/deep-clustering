...
(cropped, resized) rgb data from jpeg files.
 Output labels are int arrays of shape (input_size[0], input_size[1], 1), where each pixel holds an integer 
value representing a specific instance in the image. for example - all pixels belonging to a specific
instance of a dog, will hold the same integer value.
'''
...
def read_labeled_image_list(data_dir, labels_dir, data_list):
    ...
    for line in f:
        ...
        masks.append(labels_dir + name + '.png')
    return images, masks
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, labels_dir, data_list, input_size,
                 random_scale, random_mirror, ignore_label):
